...
const TRUSTANT_MANAGED_ENV_NAMES = new Set([
  "OPS_USER",
  "OPS_PASSWORD",
  "OPS_APIHOST",
  "OPS_REPO",
...
export function normalizeSecretName(secret: string): string {
  ...
  if (TRUSTANT_MANAGED_ENV_NAMES.has(name)) {
    throw new Error(`'${name}' is a Trustant-managed runtime variable and cannot be used as an application secret or action parameter`)
  }
  return name
}
...
export function hasEnvSecret(secret: string, path = ".env"): boolean {
  ...
  // reserved for the Trustant UI and is intentionally absent from this MCP.
 ...
 * bindings of Trustant-managed variables such as OPS_APIHOST.
 */
